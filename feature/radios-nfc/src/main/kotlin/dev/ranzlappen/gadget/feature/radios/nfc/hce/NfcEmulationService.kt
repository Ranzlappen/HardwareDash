...
class NfcEmulationService : HostApduService() {
    ...
    @Inject
    lateinit var hceState: NfcHceState
    ...
    private fun handleSelect(apdu: ByteArray): ByteArray {
        ...
        if (p1 == 0x04) {
            return if (data.contentEquals(NDEF_AID)) { selectedFile = null; SELECT_OK }
            else FILE_NOT_FOUND
        }
        if (data.size == 2) {
            return when {
                data.contentEquals(CC_FILE_ID) -> { selectedFile = CC_FILE_ID; SELECT_OK }
                data.contentEquals(NDEF_FILE_ID) -> { selectedFile = NDEF_FILE_ID; SELECT_OK }
                else -> FILE_NOT_FOUND
            }
        }
    ...
    private fun buildCcFile(): ByteArray {
        val ndefFile = buildNdefFile() ?: return byteArrayOf(
            ...
            0x04, 0x06, 0xE1.toByte(), 0x04, 0x00, 0x00, 0x00, 0xFF.toByte()
        )
    ...
    private fun buildNdefFile(): ByteArray? {
        val ndef = hceState.payload.value ?: return null
        val len = ndef.size
