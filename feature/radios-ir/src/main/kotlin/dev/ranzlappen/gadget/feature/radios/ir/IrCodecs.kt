...
/**
 * IR protocol encoder — migrated from `app/src/main/java/com/gadget/ir/IrCodecs.kt`.
 * Supports NEC, Pronto (learned 0000 header), and RAW (µs mark/space pairs).
 */
object IrCodecs {
    ...
    private fun encodePronto(payload: String, repeats: Int): Result {
        ...
        fun pairsToUs(start: Int, count: Int): IntArray {
            ...
            for (i in 0 until count * 2) out[i] = (pairs[start + i] * periodUs).toInt().coerceAtLeast(1)
            return out
        }
