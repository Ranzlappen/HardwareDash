...
/**
 * Home-screen monitor widget for `vibration_amplitude` — a determinate
 * [android.widget.ProgressBar] of the latest sample plus a monitoring toggle +
 * reload. Near-verbatim copy of torch's `MonitorWidgetProvider`: it reads the
 * shared metric config, not a per-`appWidgetId` [VibrationWidgetConfig], so it
 * stays a standalone [AppWidgetProvider] (not a [dev.ranzlappen.gadget.core.widgetkit.provider.BaseGadgetWidgetProvider]).
 *
...
class MonitorWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        private const val TAG = "VibrationMonitorWidget"
        private const val DEFAULT_MAX = 100f
        private val METRIC_KEY = VibrationMetricSource.METRIC_KEY

        const val ACTION_MONITOR_TOGGLE =
            "dev.ranzlappen.gadget.feature.vibration.ACTION_MONITOR_TOGGLE"
        const val ACTION_MONITOR_RELOAD =
            "dev.ranzlappen.gadget.feature.vibration.ACTION_MONITOR_RELOAD"
