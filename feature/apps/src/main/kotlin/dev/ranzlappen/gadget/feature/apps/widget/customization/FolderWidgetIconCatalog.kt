package dev.ranzlappen.gadget.feature.apps.widget.customization
...
/**
 * Curated registry of icons available to folder widget configurations, plus the
 * import/resolve surface for user-supplied custom icons. Built-in entries map
 * each [MaterialSymbol] to its vendored drawable (usable by RemoteViews).
 * Custom icons are downscaled WEBP copies in `filesDir/widget_icons/apps/`
 * keyed `custom:<uuid>.webp`. Mirrors [VibrationIconCatalog]'s pattern.
 */
@Singleton
class FolderWidgetIconCatalog @Inject constructor(
    ...
    val entries: List<Entry> = MaterialSymbol.entries.map { sym ->
        Entry(
            key = sym.id,
            drawable = sym.drawableRes,
            displayName = sym.id.split('_').joinToString(" ") { it.replaceFirstChar(Char::uppercase) },
        )
    }
    ...
    fun resolveSource(key: String): WidgetIconSource =
        if (isCustom(key)) {
            WidgetIconSource.CustomFile(
                File(customDir, key.removePrefix(WidgetIconKeys.CUSTOM_PREFIX)).absolutePath,
            )
        } else {
    ...
    override fun resolve(key: String): Int =
        entries.firstOrNull { it.key == key }?.drawable ?: MaterialSymbol.Folder.drawableRes
    ...
    suspend fun importCustomIcon(uri: Uri): String? = withContext(Dispatchers.IO) {
        try {
            ...
            if (bytes == null || bytes.isEmpty()) {
                Log.w(TAG, "importCustomIcon: empty/unreadable stream for $uri")
                return@withContext null
            }
            val decoded = decodeDownscaled(bytes, MAX_ICON_PX)
            if (decoded == null) {
                Log.w(TAG, "importCustomIcon: undecodable image for $uri")
                return@withContext null
            }
            ...
            bitmap.recycle()
            Log.d(TAG, "importCustomIcon: wrote ${file.name}")
            WidgetIconKeys.CUSTOM_PREFIX + file.name
        } catch (t: Throwable) {
            Log.w(TAG, "importCustomIcon failed for $uri", t)
            null
        }
    }
    ...
    private fun applyExifOrientation(bitmap: Bitmap, bytes: ByteArray): Bitmap {
        ...
        return try {
            ...
            Log.w(TAG, "applyExifOrientation OOM — using unrotated icon", e)
            bitmap
        }
    }
    ...
    companion object {
        ...
        private const val CUSTOM_DIR_NAME = "widget_icons/apps"
        private const val MAX_ICON_PX = 192
        private const val ICON_QUALITY = 80
        private const val TAG = "FolderWidgetIconCatalog"
    }
}
