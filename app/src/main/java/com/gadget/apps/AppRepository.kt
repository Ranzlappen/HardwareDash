 ...
 *  - explicit `requestRefresh()` from `MainActivity.onCreate`
 ...
 * package events triggers at most ~2 sequential scans. Web-link records
 * (added in batch 3) are NOT touched by this repository — they're owned by
 * the web-link table.
 */
@Singleton
class AppRepository @Inject constructor(
    ...
    init {
        ...
        // process before MainActivity still see a populated cache.
        refreshTrigger.trySend(Unit)
    }

    /** Public entry point used by `MainActivity.onCreate` to force a fresh scan. */
    fun requestRefresh() {
        refreshTrigger.trySend(Unit)
    }
