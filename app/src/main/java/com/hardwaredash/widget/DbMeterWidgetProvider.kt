...
import com.hardwaredash.services.DbMeterService

class DbMeterWidgetProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == ACTION_TOGGLE) {
            val nowActive = DbMeterService.toggle(context)
            WidgetActionHandler.showToast(
                context,
                if (nowActive) "dB meter started" else "dB meter stopped"
            )
        }
    }

    companion object {
        private const val ACTION_TOGGLE = "com.hardwaredash.widget.ACTION_DB_METER_TOGGLE"
        ...
        private fun setupWidget(context: Context, manager: AppWidgetManager, id: Int) {
            ...
            views.setTextViewText(R.id.widget_action_label, "-- dB")

            val intent = Intent(context, DbMeterWidgetProvider::class.java).apply {
