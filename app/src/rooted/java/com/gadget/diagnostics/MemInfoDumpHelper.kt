...
internal const val MEMINFO_TAIL_CAP_BYTES = 16 * 1024

private const val LOGBOOK_DIR_NAME = "logbook"
private const val MEMINFO_FILENAME_PREFIX = "meminfo-"
private const val MEMINFO_FILENAME_EXTENSION = ".json"
 ...
 */
@Singleton
class MemInfoDumpHelper @Inject constructor(
    private val shell: RootShell,
    @ApplicationContext private val context: Context,
) {
    suspend fun snapshot(): String? {
        val result = shell.exec(
            "dumpsys meminfo 2>/dev/null | tail -c $MEMINFO_TAIL_CAP_BYTES",
        )
        if (!result.isSuccess) return null
        return result.stdout.joinToString("\n").take(MEMINFO_TAIL_CAP_BYTES)
    }
    ...
    fun persistToLogbook(excerpt: String): File? {
        ...
        val file = File(
            dir,
            "$MEMINFO_FILENAME_PREFIX${filenameTimestamp()}$MEMINFO_FILENAME_EXTENSION",
        )
