...
internal const val PROP_DUMP_TAIL_CAP_BYTES = 16 * 1024

private const val LOGBOOK_DIR_NAME = "logbook"
private const val PROP_DUMP_FILENAME_PREFIX = "adb-properties-"
private const val PROP_DUMP_FILENAME_EXTENSION = ".json"
...
class PropDumpHelper @Inject constructor(
    ...
    suspend fun snapshot(): String? {
        val result = shell.exec("getprop 2>/dev/null | tail -c $PROP_DUMP_TAIL_CAP_BYTES")
        if (!result.isSuccess) return null
        return result.stdout.joinToString("\n").take(PROP_DUMP_TAIL_CAP_BYTES)
    }
    ...
    fun persistToLogbook(excerpt: String): File? {
        ...
        val file = File(
            dir,
            "$PROP_DUMP_FILENAME_PREFIX${filenameTimestamp()}$PROP_DUMP_FILENAME_EXTENSION",
        )
