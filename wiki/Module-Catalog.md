...
| `core:permissions` | 7 | **Built (W5).** Centralized permission framework: per-feature `@IntoMap` `FeaturePermissions` registry + app baseline (`PermissionRegistry`), `SpecialPermissions` (overlay / exact-alarm / WRITE_SETTINGS / notification-listener / all-files → live query + Settings deep-link), and a `PermissionsDashboardCard` rendered in Settings via a `permissionsSection` slot. | `:core:ui` |
...
| `feature:apps` | 40 | ✅ | App-Organizer (folders + folder widgets + canvas background renderer); content-widget archetype; `apps_folder_count` `MetricSource` + `AppsActionHandler` (`refresh_apps`/`open_folder`/`launch_app`); per-app overflow menu (rooted flavor only) for freeze/unfreeze/force-stop via `AppsRootController`. |
| `feature:apps-rooted` | 2 | ✅ | pm-based freeze (`pm disable-user`) / unfreeze (`pm enable`) / force-stop (`am force-stop`) behind `RootFeatureKey`s (`AppsFreezeApp`/`AppsUnfreezeApp`/`AppsForceStopApp`); `apps_root` ActionHandler. `AppsRootController` interface + standard no-op live in `:feature:apps` (rooted/standard split, mirroring `StorageController`); dynamic deny-list protects core system packages + this app's own running package. No dedicated screen — surfaced as rows in `:feature:apps`'s existing app list. |
...
| `feature:automation` (+ `-rooted` 4) | 5 | ✅ | Screenless by design — rooted power-user tools (privileged intent fire / allow-listed settings override / dumpsys snapshot) exposed only via `automation_extras` `ActionHandler`, so they surface as rule-builder actions in `feature:automation-ui`'s existing action picker. Resolves the former "confusing empty twin" question: not a competing UI, just the engine's own rooted-capability contributor. |
...
| `feature:microphone` (+ `-rooted` 5) | 12 | ✅ | Rooted-only "extreme mic tools" screen — gain-boost / direct-PCM capture / custom-sample-rate (confirm dialog, kernel-lockup risk) / multi-mic raw / disable-hardware-effects / system-audio-capture (mandatory legal disclaimer) rows via `MicrophoneController`; `MicrophoneActionHandler` (6 actions, all `requiresRoot`). Deliberately does not duplicate `feature:audio`'s dB meter/recording UI. MetricSource-exempt — no continuous signal, every action is fire-and-forget. |
...
| `feature:display` (+ `-rooted` 5) | 10 | ✅ | Brightness slider (standard, `Settings.System.SCREEN_BRIGHTNESS` read/write with a WRITE_SETTINGS deep-link flow) + refresh-rate/density override + read-only SurfaceFlinger snapshot (rooted) via `DisplayController`; `screen_brightness` `MetricSource`; `DisplayActionHandler` (5 actions). |
...
| `feature:notification` (+ `-rooted` 4) | 16 | ✅ | Channel inspector + simplified test-notification builder (standard) + sticky-channel-importance override / listener-access opt-in / bounded lock-screen-overlay test panel (rooted, via `NotificationController`); own `GadgetNotificationListenerService` (real, manifest-declared — unlocks `active_notifications` `MetricSource`); `NotificationActionHandler` (7 actions). Legacy `BuilderPresetStore`/`NotificationPreviewCard` were already deleted in the W1 purge (commit `3908ed2`) — nothing was ported, only the concept rebuilt. |
| `feature:adbdebug` (+ `-rooted` 5) | 12 | ✅ | ADB-enabled readout + Developer-options deep-link (standard); toggle / over-network port / `getprop` dump (optional device-storage persist) / allow-listed `setprop` editor (rooted) via `AdbDebuggingController`; `adb_enabled` `MetricSource`; `AdbDebugActionHandler` (4 actions). |
| `feature:usbdebug` (+ `-rooted` 5) | 11 | ✅ | USB-debugging-enabled readout + Developer-options deep-link (standard, reads the same `ADB_ENABLED` setting `adbdebug` does under its own concept); USB function-role picker (7 chips) + labelled "USB Diagnostics" panel (usb/serial-service/debugfs, each tagged by probe source) via `UsbDebuggingController`; `usb_debugging` `MetricSource`; `UsbDebugActionHandler` (4 actions). |
...
| `feature:radios-cell` (+ `-rooted` 2) | 11 | ✅ | Standard `TelephonyManager`/`TelephonyCallback` screen — SIM state, carrier name, network-type badge, live signal bars (READ_PHONE_STATE-gated) — the first module in the modular codebase built with no standard-tier precedent to wrap; rooted "raw modem diagnostics" panel over `CellController`'s read-only Qualcomm dumps. `cell_signal` `MetricSource` (push, live). `CellActionHandler` — assert-signal-above/below (read-only module, no write path by design). |
...
> _Last reviewed: 2026-07-11 · Source: `settings.gradle.kts`, live
